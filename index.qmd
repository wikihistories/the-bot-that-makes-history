...
Yellowdesk's guidelines crystallise a radical ideal, which was later encoded in TedderBot and Yapperbot. According to Yellowdesk, Template:Current has nothing to do with the currency of an event. It is purely an "advisory to editors" when an article is being intensely edited. Editors may be tempted to mark an article as current when the article's subject-matter is in the present, but this would clutter the encyclopaedia; for Yelowdesk, it is obvious that having the template on "hundreds of thousands of articles" at a time would be unsightly. To mollify editors who wish to mark the currency of an article, Yellowdesk points to other systems in the encyclopaedia. Yellowdesks's original guidelines mention the [Current events page](https://en.wikipedia.org/wiki/Portal:Current_events); recent revisions also mention the [In the News](https://en.wikipedia.org/wiki/Wikipedia:In_the_news) section of the [Main Page](https://en.wikipedia.org/wiki/Main_Page).
...
This example undermines @de_laat_use_2015's argument that bots are unable to exercise "moral skills." He argues that only humans can be "tactful, forgiving and supportive," and implies that tact, forgiveness, and support are the essential moral skills for the deliberation of an encylcopaedia. Tact and support are certainly virtues of the bots: their comments are polite, clear, and link to authoritative sources where humans can educate themselves. Forgiveness is a difficult virtue for a bot to exercise, because it cannot take offence. But seen more broadly, forgiveness is a kind of pliability or softness, and both TedderBot and Yapperbot are in fact pliable. Yapperbot indicated as much in its original edit comment: "Did I get this wrong? Please revert me and check my userpage!" In its initial configuration, Yapperbot actively invited disagreement. Once its trial period was over, and it was fully approved, its comment became more self-certain.
...
To conclude, I want to consider the nature of time on Wikipedia. Yapperbot constantly contradicts itself, arguing that an article can be "current" without being "{{current}}". This contradiction is apparently unique to English Wikipedia. On French Wikipedia, for example, Template:Current is simply used to highlight the presentness of an event. The French Template:Current is divided into two modes, "Événement récent" (recent event) and "Événement en cours" (ongoing event) [@noauthor_modeevenement_2023]. Editors can use these templates simply to indicate whether an article describes an event that finished recently (récent) or is still "unrolling itself" (se déroule) in the present. The French Template:Current is not regulated by a bot. It defines "en cours" or "récent" according to ordinary French usage, rather than using a technical definition based on edit volumes. French Wikipedia seems to be typical. I have not been able to locate a language edition that resembles English in its strict definition and enforcement of "currency."
