...
# Import data
DATA_DIR <- "data"
bots <- file.path(DATA_DIR, "bots.csv") |> readr::read_csv()
yellowdesk <- file.path(DATA_DIR, "yellowdesk.csv") |> readr::read_csv()
```
...
```{r}
#| label: fig-daily-frequency-descriptive
#| fig-cap: "Descriptive statistics for daily uncurrenting"
bots |> 
  dplyr::group_by(user) |> 
  dplyr::count(date) |> 
  dplyr::group_by(user) |> 
  dplyr::summarise(
    mean = mean(n, trim = 0.02),
    median = median(n),
    max = max(n),
    days_in_operation = min(date) %--% max(date) |> lubridate::time_length(unit = "day"),
    days_without_edit = days_in_operation - n(),
    prop_without_edit = days_without_edit / days_in_operation
  )
```
...
```{r}
#| label: fig-n-per-day
#| fig-cap: "Both TedderBot and Yapperbot uncurrent a similar number of articles per day"
bots |> 
  dplyr::group_by(user) |> 
  dplyr::count(date) |> 
  ggplot2::ggplot(aes(x=n)) +
  ggplot2::geom_density(stat="density", n=512) +
  ggplot2::scale_x_log10() +
  ggplot2::facet_wrap(vars(user)) +
  ggplot2::labs(
    x = "Articles uncurrented per day (log scale)",
    y = "Density estimate"
  )
```

What were the five most active days?

```{r}
most_active_day <- bots |> 
  dplyr::filter(user == "Yapperbot") |> 
  dplyr::group_by(date) |> 
...
bots |> 
  dplyr::filter(user == "Yapperbot") |> 
  dplyr::filter(date == most_active_day) |> 
  dplyr::select(pageid, title, timestamp)
```
...
bots |> 
  ...
  dplyr::summarise(num_pages = n())
```

Which page had to be uncurrented nine times?

```{r}
dplyr::filter(bots, user == "Yapperbot") |> 
  dplyr::group_by(pageid, title) |> 
  dplyr::summarise(count = n()) |> 
